...
def job_config = [
        job: [
                name: "asdasfasdasd2_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'asdasfasdasd2_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/asdasfasdasd2_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.asdasfasdasd2.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "asdasfasdasd2", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("asdasfasdasd2_Pipeline_develop");

job.with {

    authenticationToken('rPcvYO5qhYuhKn4uDsBlYwAuhks9FSeU')
